...
Image resize_4(Image a)
{
    ...
    int size_by_4 = (a.width/4 * a.height/4);
    printf("Image A width %d height %d total size %d, total size divided by 4 %d, components of a_16 are %d\n", a.width, a.height, (a.width * a.height), size_by_4 , (len/16)  );
        printf("Image C width %d height %d total size %d components of c are %d\n", c.width, c.height, (c.width * c.height), c_len);
    int j = 0 ; //Keeps track of position of c
